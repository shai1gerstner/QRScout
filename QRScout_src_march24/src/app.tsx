...
export default function App() {
  ...
  // Only initialize Statsig if key exists
  const statsig = statsigKey
    ? useClientAsyncInit(statsigKey, { userID: `${teamNumber}` })
    : null;

  const client = statsig?.client;
  ...
  // If no key, just run without Statsig
  if (!statsigKey) return content;

  // If key exists, wrap with StatsigProvider but DO NOT block UI forever
  return (
    <StatsigProvider client={client!} loadingComponent={content}>
      {content}
    </StatsigProvider>
  );
}
